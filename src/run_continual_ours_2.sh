...
ENCODER_FREEZE_MODE='first_appearance' # 'all_trainable' #'first_appearance'

# CL-method scope: where the cl_method-specific machinery applies. Values:
#   moe (default):       only MoE expert weights are protected.
#   moe_and_encoder:     extend the same protection to encoder nn.Linear
#                        and nn.Conv1d(k=1) layers, in the cl_method's
#                        native style:
#                          ours -> StackedLowRank* per-stage components.
#                          lora -> per-stage LoRA adapters on a frozen
#                                  stage-0 base.
#                          ewc  -> Fisher penalty extended to encoder
#                                  weights (no structural change).
#                        Overrides --encoder_freeze_mode for layers it
#                        touches.
