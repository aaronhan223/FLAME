...
def data_prepare(args, task, tokenizer, modeltype, data=None):
    """
    ...
    test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=args.eval_batch_size, collate_fn=TextTSIrgcollate_fn)
...
class TSNote_Irg(Dataset):
    ...
    def __init__(self, args, mode, task, tokenizer, modeltype, data=None):
        self.tokenizer = tokenizer
        self.max_len = args.max_length
        if task == 'ihm' or task == 'los':
            self.task = task + '-48-cxr-notes-ecg'
        else:
            self.task = task + '-all-cxr-notes-ecg'
        if data != None:
            self.data = data
        else:
    ...
    def __getitem__(self, idx):
        ...
        reg_ts=np.array(data_detail['reg_ts'])
        ...
        if 'CXR' in self.modeltype and not data_detail['cxr_missing']:
            cxr_feats = np.array(data_detail['cxr_feats'])
        ...
        if 'ECG' in self.modeltype and not data_detail['ecg_missing']:
            ecg_feats = np.array(data_detail['ecg_feats'])
        ...
        elif self.modeltype == 'CXR_TS_Text':
            return {'idx': idx, 'ts': ts, 'ts_mask': ts_mask, 'ts_tt': ts_tt, 'reg_ts': reg_ts, "input_ids": text_token, "label": label, "attention_mask": atten_mask, "text_embeddings": text_emb, \
        ...
        elif self.modeltype == 'CXR_ECG_TS_Text':
            return {'idx': idx, 'ts': ts, 'ts_mask': ts_mask, 'ts_tt': ts_tt, 'reg_ts': reg_ts, "input_ids": text_token, "label": label, "attention_mask": atten_mask, "text_embeddings": text_emb, \
            'note_time': text_time_to_end, 'text_time_mask': text_time_mask, 'text_missing': data_detail['text_missing'],
...
def TextTSIrgcollate_fn(batch):
    ...
    if 'note_time' in batch[0].keys():
        note_time=pad_sequence([example['note_time'].clone().detach() for example in batch],batch_first=True,padding_value=0)
        note_time_mask=pad_sequence([example['text_time_mask'].clone().detach() for example in batch],batch_first=True,padding_value=0)
    else:
    ...
    if 'cxr_feats' in batch[0].keys():
        ...
        cxr_time = pad_sequence([example['cxr_time'].clone().detach() for example in batch], batch_first=True, padding_value=0)
        cxr_time_mask = pad_sequence([example['cxr_time_mask'].clone().detach() for example in batch], batch_first=True, padding_value=0)
    else:
