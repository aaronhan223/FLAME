...
def test_iter_encoder_target_weights_scope_and_dedup():
    """``iter_encoder_target_weights`` covers Linear + Conv1d(k=1), skips
    Conv1d(k>1) and LayerNorm, dedups shared encoders by ``id``."""
    class Enc(torch.nn.Module):
        def __init__(self, D=8):
            super().__init__()
            self.fc = torch.nn.Linear(D, D)
            self.c1 = torch.nn.Conv1d(D, D, kernel_size=1)
            self.c3 = torch.nn.Conv1d(D, D, kernel_size=3, padding=1)
            self.norm = torch.nn.LayerNorm(D)

    enc = Enc()
    pairs = list(iter_encoder_target_weights({'A': enc, 'B': enc, 'C': enc}))
    names = [n for n, _ in pairs]
    print(f'  yielded {len(pairs)} encoder target weights: {names}')
    # Two weights expected: fc.weight + c1.weight. Conv1d(k=3) and LayerNorm excluded.
    assert len(pairs) == 2, f'expected 2 weights, got {len(pairs)}'
    assert all(n.startswith('__encoder__.') for n, _ in pairs)
    assert any(n.endswith('.fc.weight') for n, _ in pairs)
    assert any(n.endswith('.c1.weight') for n, _ in pairs)
    print('  Linear + Conv1d(k=1) yielded; Conv1d(k=3) + LayerNorm skipped  [OK]')


def test_make_combined_target_iterator_covers_both():
    """The combined factory yields from base_iterator(model) AND from
    iter_encoder_target_weights(encoders)."""
    sm = _make_seq_moe(num_experts=2, embed_dim=8, hidden=16, ks=3)
    layer = _Layer(sm)
    install_continual_routers(layer, mode='per_task_router', combine='mean')
    ...
    encoders = {'A': Enc()}
    combined = make_combined_target_iterator(iter_expert_weights, encoders)
    pairs = list(combined(layer))
    n_encoder = sum(1 for n, _ in pairs if n.startswith('__encoder__.'))
    n_expert = sum(1 for n, _ in pairs if '.moe.experts.' in n)
    print(f'  combined yields {n_expert} expert + {n_encoder} encoder entries')
    assert n_expert > 0 and n_encoder == 1
    print('  combined iterator unions expert + encoder targets  [OK]')


def test_ewcstate_with_combined_iterator_covers_encoder_drift():
    """Encoder weights drifted -> regularizer grows when EWCState was
    constructed with the combined iterator."""
    sm = _make_seq_moe(num_experts=2, embed_dim=8, hidden=16, ks=3)
    layer = _Layer(sm)
    install_continual_routers(layer, mode='per_task_router', combine='mean')
    ...
    encoders = {'A': Enc()}
    combined = make_combined_target_iterator(iter_expert_weights, encoders)
    state = EWCState(layer, target_iterator=combined)
    # Set Fisher to identity for every tracked param so reg = 0.5 * ||Δ||² .
    for n, f in state.fisher.items():
        state.fisher[n] = torch.ones_like(f)

    reg0 = state.regularizer(layer).item()
    # Drift the encoder fc weight by +0.5.
    with torch.no_grad():
        encoders['A'].fc.weight.add_(0.5)
    reg1 = state.regularizer(layer).item()
    print(f'  reg before: {reg0:.4f}, after encoder drift: {reg1:.4f}')
    assert reg1 > reg0, 'regularizer did not respond to encoder drift'
    print('  EWC penalty extends to encoder weights via combined iterator  [OK]')
...
def main():
    ...
    print('\niter_encoder_target_weights scope + dedup')
    print('-' * 60)
    test_iter_encoder_target_weights_scope_and_dedup()

    print('\nmake_combined_target_iterator unions expert + encoder targets')
    print('-' * 60)
    test_make_combined_target_iterator_covers_both()

    print('\nEWCState with combined iterator catches encoder drift')
    print('-' * 60)
    test_ewcstate_with_combined_iterator_covers_encoder_drift()
