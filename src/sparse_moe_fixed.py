...
# from src.module import MoEConfig
import pdb
...
class SparseDispatcher(object):
    ...
    def combine(self, expert_out, multiply_by_gates=True):
        ...
        """
        # FIX: no .exp() needed — expert outputs are already in linear space
        ...
        # FIX: no .log() needed — stay in linear (embedding) space
        return combined
...
class MLP(nn.Module):
    def __init__(self, config:MoEConfig, input_size:int, output_size:int, hidden_size:int):
        ...
        # FIX: zero-init final layer so MoE starts as identity (no-op)
        # and the optimizer can grow the MoE contribution
    ...
    def forward(self, x):
        ...
        out = self.fc2(out)
        # FIX: removed LogSoftmax — output stays in embedding space so it can
        # be added to a residual stream without sign/scale mismatch
        return out
