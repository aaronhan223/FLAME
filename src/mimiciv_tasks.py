...
def main():
    ...
    if args.transfer_moe:
        savedir = f'./checkpoints/flame/multitask/{args.base_task}/{args.base_task_mods}/experts_{args.num_of_experts[0]}/{args.seed}/{args.task}_{task_mods}_transfer_moe_from_{args.base_task}.pt'
        os.makedirs(os.path.dirname(savedir), exist_ok=True)
    elif args.lora:
        savedir = f'./checkpoints/{args.fusion_model}/{args.base_task}/{args.base_task_mods}/experts_{args.num_of_experts[0]}/{args.seed}/{args.task}_{task_mods}_lora_from_{args.base_task}.pt'
        os.makedirs(os.path.dirname(savedir), exist_ok=True)
    elif args.fine_tune:
        savedir = f'./checkpoints/{args.fusion_model}/{args.base_task}/{args.base_task_mods}/experts_{args.num_of_experts[0]}/{args.seed}/{args.task}_{task_mods}_ft_from_{args.base_task}.pt'
        os.makedirs(os.path.dirname(savedir), exist_ok=True)
    ...
    # checkpoints. Use a directory level (not a filename suffix) — the existing
    # filename is already close to the 255-byte single-name limit on ext4 and
    # an extra suffix overflows it. Default ('shared') leaves the path
    # unchanged.
    if args.pheno_encoder == 'separate' and 'pheno' in args.task.split('-'):
        savedir = os.path.join(
            os.path.dirname(savedir), 'pheno_enc_separate', os.path.basename(savedir)
        )
        os.makedirs(os.path.dirname(savedir), exist_ok=True)
