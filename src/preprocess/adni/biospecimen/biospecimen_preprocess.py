...
# Required Files
bio_1 = pd.read_csv('/export/io79/data/schaud35/datasets/adni/biospecimen/APOERES_09Apr2026.csv')
bio_2 = pd.read_csv('/export/io79/data/schaud35/datasets/adni/biospecimen/UPENNBIOMK_ROCHE_ELECSYS_09Apr2026.csv')
label = pd.read_csv('/export/io79/data/schaud35/datasets/adni/diagnosis/DXSUM_20Apr2026.csv')


# Normalize headers once so downstream access is consistent.
bio_1.columns = bio_1.columns.str.upper().str.strip()
bio_2.columns = bio_2.columns.str.upper().str.strip()
label.columns = label.columns.str.upper().str.strip()
...
def process_num_cat_cols(_df, initial_filling=None, col_dict=None):
    ...
    if len(numerical_col_idx) > 0:
        ...
        for column in df_num.columns:
            df_num[column] = df_num[column].apply(_safe_to_float)
...
for i in range(tmp.shape[0]):
    if pd.notna(tmp.loc[i, 'PHASE']):
        if pd.notna(tmp.loc[i, 'RID']):
            _tmp = str(tmp.loc[i]['PHASE'])[-1]+'_'+str(tmp.loc[i]['RID'])
            id_list.append(_tmp)
...
df_label.to_csv('/export/io79/data/schaud35/datasets/adni_processed/label.csv')
...
final.to_csv('/export/io79/data/schaud35/datasets/adni_processed/biospecimen_merged.csv', index=True)
...
final_2.to_csv('/export/io79/data/schaud35/datasets/adni_processed/biospecimen_merged_mean.csv', index=True)
